...
Point your local opencode config at the source file when you want changes picked up immediately during development. In `~/.config/opencode/opencode.json`:
...
| Command | Description |
|---------|-------------|
| `bun run build` | Compile runtime JavaScript and declarations into `dist/` |
...
2. Make your changes and ensure `bun run build`, `bun run typecheck`, and `bun test` pass
